...
nrow(ncbi_CDI)
```
We have `r nrow(all_ncbi_CDI)` samples for which we have calculated all chosen alpha metrics.
...
```{r}
# Clean and filter metadata
hosp_CDI_meta <- dplyr::select(hosp_CDI_meta_all, Sample.ID, sample.origin, sex, age, time, CDItreatment.PREFMT, severeICD, Clinicalrecurrence)
...
hosp_CDI <- hosp_CDI[complete.cases(hosp_CDI),]

for(i in 1:nrow(hosp_CDI)) {
  if(hosp_CDI$day_since_fmt[i] <= 0){
