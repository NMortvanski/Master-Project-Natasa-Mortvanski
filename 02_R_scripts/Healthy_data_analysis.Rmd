...
table_saphiro$p.value <- round(table_saphiro$p.value, digits = 4)
...
for (i in 1:length(metric)){
  descdist(healthy_disease[[metric[i]]], discrete = FALSE)
}
```

the p-value < 0.05 implying that the distribution of the data are significantly different from normal distribution. 
In other words, we cannot assume the normality for neither of the alpha metrics.
