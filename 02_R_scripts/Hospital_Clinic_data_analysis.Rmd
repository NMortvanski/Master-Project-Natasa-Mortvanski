...
for(i in 1:nrow(hospital_CDI)) {
  if(hospital_CDI$day_since_fmt[i] <= 0){
    hospital_CDI$FMT_pre_post[i] <- "pre"
    hospital_CDI$condition[i] <- paste(hospital_CDI$condition[i], "pre", sep="_")
  } else {
    hospital_CDI$FMT_pre_post[i] <- "post"
    hospital_CDI$condition[i] <- paste(hospital_CDI$condition[i], "post", sep="_")
  }
}
```

Load Hospital Clínic's fecal donor data:
...
compare_hospital <- rbind.fill(hosp_donor, hospital_CDI_pre_FMT)
...
```{r}
all_data_comparison <- rbind.fill(all_healthy, CDI, hosp_donor, hospital_CDI)
...
# need to do produce differentials.qza in QIIME2 first (qiime aldex2 aldex2)
...
```{r}
top_four <- top_taxa(physeq.fam.rel, 15)
top_four
```
...
metadata_donor <- hosp_donor_meta
...
```{r}
# Keep samples with non-zero total taxa
pruned <- prune_samples(sample_sums(physeq_merged) > 0, physeq_merged)

# Check total taxa in each sample again
sample_sums(pruned)

#rarefy samples
physeq_rarefy_merged <- rarefy_even_depth(pruned, rngseed=1, sample.size=0.9*min(sample_sums(pruned)), replace=F)

#Check the taxa prevalence at Phylum level
plot_taxa_prevalence(physeq_rarefy_merged, "Order")
```
...
physeq.genus <- aggregate_taxa(physeq_merged, "Genus")
top_four <- top_taxa(physeq.genus, 4)
top_four
